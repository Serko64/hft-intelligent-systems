...
def run_worker(args: tuple) -> tuple[float, np.ndarray, float, float]:
    ...
    for step in range(n_steps):
        ...
        if np.random.random() < epsilon:
            ...
            with torch.no_grad():
                q_values = online_net(torch.from_numpy(
        ...
        if buffer_size >= batch_size and step % TRAIN_FREQ == 0:
            ...
            batch_states = torch.from_numpy(
                replay_states[batch_indices]).to(device)
            batch_next_states = torch.from_numpy(
                replay_next_states[batch_indices]).to(device)
            batch_actions = torch.from_numpy(
                replay_actions[batch_indices]).to(device)
            batch_rewards = torch.from_numpy(
                replay_rewards[batch_indices]).to(device)
            batch_dones = torch.from_numpy(
                replay_dones[batch_indices]).to(device)
            ...
            with torch.no_grad():
                ...
                td_target = batch_rewards + GAMMA * \
                    (1.0 - batch_dones) * next_q
